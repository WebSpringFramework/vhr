...
(
    id           int auto_increment
        primary key,
    eid          int          null,
    asDate       date         null comment 'Salary adjustment date',
    beforeSalary int          null comment 'Salary adjustment',
    afterSalary  int          null comment 'Adjusted salary',
    reason       varchar(255) null comment 'Reason for salary adjustment',
    remark       varchar(255) null comment 'Remarks',
...
(
    id         int auto_increment
        primary key,
    eid        int          null,
    appDate    date         null comment 'Evaluation date',
    appResult  varchar(32)  null comment 'Evaluation results',
...
begin
    ...
    declare pid int;
    declare pcount int;
    declare a int; select count(*) into a from department where id = did and isParent = false; if a = 0 then
    set result = -2; else
    select count(*) into ecount from employee where departmentId = did; if ecount > 0 then
    set result = -1; else
    select parentId into pid from department where id = did;
    delete from department where id = did and isParent = false; select row_count() into result; select count(*) into pcount from department where parentId = pid; if pcount = 0 then
